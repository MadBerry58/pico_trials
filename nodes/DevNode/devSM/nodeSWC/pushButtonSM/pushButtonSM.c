...
uint8_t pushButtonSM_init(pushButtonSM_type sm)
{
  ...
  else
  {
    ...
    sm.pushButtonState = INITIALIZED;                                        /* Set SM state to INITIALIZED */
  }
  ...
  while(loop)
  {
    switch(sm.pushButtonState)                                                   /* Switch state machine behavior */
    {
      ...
      case INITIALIZED:
        ...
        loop = true;                                                             /* Transition to RELEASED state */
      break;

      case RELEASED:
        if(true == gpio_get(sm.buttonPin))                                       /* Button pressed */
        {
          sm.debounceTimeout_us = (time_us_32() + sm.debounceTimeC_us);        /* Initiate debounce timeout value */
