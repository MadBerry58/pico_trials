...
static inline UserInput checkUpdate_encoderSM()
{
  ...
  if(true == encoderPosition_updateFlag)
  {
    ...
    Frame_SWCdata_Updated      = true; ///TODO: separate frame update functionality
    retVal                     = ROTATE;
  }
  else
  {
...
{
  uint8_t retVal = 0;
  white_intensity = encoderPosition;
  return retVal;
}
...
uint8_t controlLogic_ledTesterSM_init()
{
  uint8_t retVal = 0;
...
uint8_t controlLogic_ledTesterSM_run()
{
  uint8_t retVal = 0u;
